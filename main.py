...
async def pdf_to_images(
    file: UploadFile = File(...), dpi: int = 200, format: str = "PNG"
):
    """
...
async def extract_text(
    ...
    try:
        ...
        data = pytesseract.image_to_data(
            image, lang=language, config=config or "", output_type=pytesseract.Output.DICT
        )
        ...
        return {
            ...
            "average_confidence": (
                sum(w["confidence"] for w in words_with_confidence)
...
async def pdf_to_text(
    file: UploadFile = File(...), language: str = "eng", dpi: int = 300
):
    """
    ...
    try:
        ...
        for i, image in enumerate(images):
            try:
                ...
                page_results.append(
                    {"page": i + 1, "text": "", "error": str(page_error)}
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"PDF text extraction failed: {str(e)}"
        )
